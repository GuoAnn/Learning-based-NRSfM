function [F,Jk] = myfun1(x,J,normalized_Image,measurements,order,lambda_reg,point_image_num)
% lambda_reg: 第一帧正则化权重
% point_image_num: 每帧的点数数组 (用于定位第一帧参数)
if nargin < 6
    lambda_reg = 0;
end
...
num_measure=size(measurements,2);
...
for i=1:1:num_measure
    for j=1:1:size(measurements{i}.point,1)
        ...
        %% Function
        F(sum(point_num(1:i-1))*4+j*4-3:sum(point_num(1:i-1))*4+j*4,1)=[1*(k1_bar-dx1_dy1*k1-dx2_dy1*k2+dy2_dx1*ddx1_dy1dy2+dy2_dx2*ddx2_dy1dy2);
...
%% ====== 新增：第一帧平面正则化 ======
% 对第一帧的所有点，惩罚 k1 和 k2 偏离零
% 这迫使第一帧保持平面
n_base_residuals = sum(point_num)*4; % 原始残差数量
n_frame1_pts = point_image_num(1);   % 第一帧点数
sqrt_lambda = sqrt(lambda_reg);

% 正则化残差: sqrt(lambda) * k1_frame1(p), sqrt(lambda) * k2_frame1(p)
F_reg = zeros(2*n_frame1_pts, 1);
row_reg = [];
col_reg = [];
val_reg = [];

for p = 1:n_frame1_pts
    idx_k1 = 2*p - 1;  % 第一帧第p个点的k1在x中的索引
    idx_k2 = 2*p;       % 第一帧第p个点的k2在x中的索引
    
    F_reg(2*p-1) = sqrt_lambda * x(idx_k1);
    F_reg(2*p)   = sqrt_lambda * x(idx_k2);
    
    % Jacobian: d(F_reg(2*p-1))/d(x(idx_k1)) = sqrt_lambda
    %           d(F_reg(2*p))/d(x(idx_k2))   = sqrt_lambda
    row_reg = [row_reg, n_base_residuals + 2*p-1, n_base_residuals + 2*p];
    col_reg = [col_reg, idx_k1, idx_k2];
    val_reg = [val_reg, sqrt_lambda, sqrt_lambda];
end
...
% 拼接Jacobian
total_rows = n_base_residuals + 2*n_frame1_pts;
Jk = sparse([row, row_reg], [col, col_reg], [val, val_reg], total_rows, size(x,1));
Jk(:,order) = [];
