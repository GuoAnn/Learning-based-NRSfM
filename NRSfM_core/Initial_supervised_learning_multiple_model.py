...
############
    ...
    for data_id in range(num_data):
        for image_id in range(num_frames):
            ...
            # Use Python spline fitting instead of MATLAB
            points_3D_np = points_3D.cpu().detach().numpy()
            fit_result = fit_python(uv, points_3D_np, uv, smoothing=1e-5, grid_size=50)
            
            dqu = torch.tensor(np.array(fit_result[1]), dtype=torch.float32).to(device)
            dqv = torch.tensor(np.array(fit_result[2]), dtype=torch.float32).to(device)
