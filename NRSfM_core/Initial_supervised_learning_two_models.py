...
##############################parameters##############################################################
    ...
    shape_partial_derivate = Fully_connection(num_point_per_frame*5, num_point_per_frame*2)
...
###################################################################################################################################################
    ...
    latent_space_const = torch.zeros(1, num_point_per_frame)
    latent_space = torch.randn(2, num_point_per_frame, requires_grad=True)
    parameters_to_optimiza = [{"params":shape_partial_derivate.parameters()},{'params': latent_space}]
    ...
    for i in range(num_iterations):
        ...
        for data_id in range(num_data):
            for image_id in range(num_frames):
                points_3D = normilized_point[image_id, [0, 1, 2], :] * random_depth_data[data_id * num_frames + image_id, :].repeat(3, 1)
                points_3D_latent = (normilized_point[image_id, [0, 1, 2], :] + torch.cat((latent_space, latent_space_const),0)) * random_depth_data[data_id * num_frames + image_id, :].repeat(3, 1)
                ...
                #y_result_latent = shape_partial_derivate.forward(X_Node, X_Neis, torch.cat((points_3D_latent, normilized_point[image_id, [0, 1, ], :]), 0))#Way 2
                ...
                y_result = shape_partial_derivate.forward(torch.flatten(torch.transpose(torch.cat((points_3D, normilized_point[image_id, [0, 1, ], :]), 0), 0, 1)))
                y_result_latent = shape_partial_derivate.forward(torch.flatten(torch.transpose(torch.cat((points_3D_latent, normilized_point[image_id, [0, 1, ], :]), 0), 0, 1)))
        ...
        points_3D_ref_latent = (normilized_point[0, [0, 1, 2], :] + torch.cat((latent_space, latent_space_const),0)) * random_depth_data[0, :].repeat(3, 1)
        #y_result_ref = shape_partial_derivate.forward(X_Node, X_Neis, torch.cat((points_3D_ref, normilized_point[0, [0, 1], :]), 0))
        #y_result_ref_latent = shape_partial_derivate.forward(X_Node, X_Neis, torch.cat((points_3D_ref_latent, normilized_point[0, [0, 1], :]), 0))#Way 1
        ...
        y_result_ref = shape_partial_derivate.forward(torch.flatten(torch.transpose(torch.cat((points_3D_ref, normilized_point[0, [0, 1, ], :]), 0), 0, 1)))
        y_result_ref_latent = shape_partial_derivate.forward(torch.flatten(torch.transpose(torch.cat((points_3D_ref_latent, normilized_point[0, [0, 1, ], :]), 0), 0, 1)))
